...
describe('Dataset Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./dataset-detail.component').then(m => m.DatasetDetailComponent),
              resolve: { dataset: () => of({ id: 123 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('Should load dataset on init', async () => {
      ...
      // THEN
      expect(instance.dataset()).toEqual(expect.objectContaining({ id: 123 }));
    });
  });
