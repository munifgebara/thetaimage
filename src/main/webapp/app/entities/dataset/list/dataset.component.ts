...
export class DatasetComponent implements OnInit {
  ...
  trackId = (_index: number, item: IDataset): number => this.datasetService.getDatasetIdentifier(item);
  ...
  protected fillComponentAttributesFromResponseBody(data: IDataset[] | null): IDataset[] {
    ...
    if (this.links().prev) {
      const datasetsNew = this.datasets ?? [];
      if (data) {
        for (const d of data) {
          if (datasetsNew.some(op => op.id === d.id)) {
            datasetsNew.push(d);
          }
        }
      }
