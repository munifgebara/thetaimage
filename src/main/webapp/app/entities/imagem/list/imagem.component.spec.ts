...
import { sampleWithRequiredData } from '../imagem.test-samples';
...
describe('Imagem Management Component', () => {
  ...
  it('Should call load all on init', () => {
    ...
    expect(comp.imagems?.[0]).toEqual(expect.objectContaining({ id: 123 }));
  });

  describe('trackId', () => {
    it('Should forward to imagemService', () => {
      const entity = { id: 123 };
      jest.spyOn(service, 'getImagemIdentifier');
      const id = comp.trackId(0, entity);
      expect(service.getImagemIdentifier).toHaveBeenCalledWith(entity);
