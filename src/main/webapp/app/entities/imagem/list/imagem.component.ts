...
export class ImagemComponent implements OnInit {
  ...
  trackId = (_index: number, item: IImagem): number => this.imagemService.getImagemIdentifier(item);
  ...
  delete(imagem: IImagem): void {
    const modalRef = this.modalService.open(ImagemDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.imagems = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: IImagem[] | null): IImagem[] {
    // If there is previous link, data is a infinite scroll pagination content.
    if (this.links().prev) {
      const imagemsNew = this.imagems ?? [];
      if (data) {
        for (const d of data) {
          if (imagemsNew.some(op => op.id === d.id)) {
            imagemsNew.push(d);
          }
        }
      }
  ...
  protected queryBackend(): Observable<EntityArrayResponseType> {
    ...
    return this.imagemService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
